...
describe('generateMCConfig', () => {
  it('replaces SERVER_NAME and TEBEX_SUBDOMAIN in output', () => {
    const out = generateMCConfig(FIXTURE_PATH, 'My Server', 'charidh', [], false)
    expect(out).toContain('My Server')
    expect(out).toContain('charidh')
    expect(out).not.toContain('{SERVER_NAME}')
    expect(out).not.toContain('{TEBEX_SUBDOMAIN}')
  })

  it('omits discord command block', () => {
    const out = generateMCConfig(FIXTURE_PATH, 'Srv', 'srv', [], false)
  ...
  it('when hasLore is true, keeps lore and sets tab_completer from overworld regions', () => {
    ...
    const out = generateMCConfig(FIXTURE_PATH, 'Srv', 'srv', regions, true)
