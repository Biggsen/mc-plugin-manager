...
conditions:
  ...
  top-explorer-2:
    ...
    yes: '2. %ajlb_lb_aach_custom_total_discovered_2_alltime_name% - %math_0_round({ajlb_lb_aach_custom_total_discovered_2_alltime_name_value}/<TOTAL_COUNT>*100,0)%%'
    no: ''
  
  top-explorer-3:
    conditions:
      - '%ajlb_lb_aach_custom_total_discovered_3_alltime_name%!=---'
    yes: '3. %ajlb_lb_aach_custom_total_discovered_3_alltime_name% - %math_0_round({ajlb_lb_aach_custom_total_discovered_3_alltime_name_value}/<TOTAL_COUNT>*100,0)%%'
    no: ''
  
  top-explorer-4:
    conditions:
      - '%ajlb_lb_aach_custom_total_discovered_4_alltime_name%!=---'
    yes: '4. %ajlb_lb_aach_custom_total_discovered_4_alltime_name% - %math_0_round({ajlb_lb_aach_custom_total_discovered_4_alltime_name_value}/<TOTAL_COUNT>*100,0)%%'
    no: ''
  
  top-explorer-5:
    conditions:
      - '%ajlb_lb_aach_custom_total_discovered_5_alltime_name%!=---'
    yes: '5. %ajlb_lb_aach_custom_total_discovered_5_alltime_name% - %math_0_round({ajlb_lb_aach_custom_total_discovered_5_alltime_name_value}/<TOTAL_COUNT>*100,0)%%'
    no: ''
```
...
- [ ] Generate scoreboard sections (conditional by world)
- [ ] Generate top explorers conditions

### Phase 2: Merge & Validation
- [ ] Implement `mergeTABConfig()` function
- [ ] Preserve static conditions from template
- [ ] Implement `validateTABDiff()` function
- [ ] Add diff gate validation to build workflow

### Phase 3: Build Integration
- [ ] Update IPC build handler to accept `tabPath`
- [ ] Add TAB generation to build process
- [ ] Update build report structure
- [ ] Generate output files with correct naming

### Phase 4: UI Updates
- [ ] Add TAB file selector to BuildScreen
- [ ] Update build validation logic
- [ ] Display TAB generation status in build report
- [ ] Show computed counts in build report

### Phase 5: Testing
- [ ] Test with overworld-only regions
- [ ] Test with overworld + nether regions
- [ ] Test with villages and hearts
- [ ] Test diff validation with various template configs
- [ ] Verify static conditions are preserved
